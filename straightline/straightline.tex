...
\documentclass[paper=6.125in:9.250in,pagesize=pdftex,
...
physical justification for the fit.\note{When fitting is used for
  ...
  method outlined in \chaptername~\ref{sec:standard} can be, under a
...
all situations in which they are used.\note{The term ``linear
  ...
  the procedure outlined in this \chaptername, but with the covariance
...
errors and absurdities in the literature.\note{Along the lines of
  ...
  this generic situation in later \chaptername s.  What is written in
...
\chapter{Standard practice}\label{sec:standard}
...
distance in the data space.\note{The inverse covariance matrix
  ...
  \chaptername~\ref{sec:twod} when we think about data for which there
  are uncertainties in both the $x$ and $y$ directions.

  Because of the purely quadratic nature of this metric distance, the
  standard problem solved in this \chaptername\ has a linear solution.
  ...
  we will see in later \chaptername s) has either of these properties,
...
\chaptername~\ref{sec:uncertainty}.
...
Using the standard linear algebra method of this \chaptername, fit the
...
Generalize the method of this \chaptername\ to fit a general quadratic
...
\chapter{The objective function}\label{sec:objective}
...
optimum.\note{Of course, whether you are a frequentist or a Bayesian,
  ...
  way.  In the fully Gaussian case described in this \chaptername,
...
\chaptername\ we did \emph{not} proceed according to these rules;
...
scientifically, the procedure of the previous \chaptername.
...
the problem\note{One oddity in this \chaptername\ is that when we
  ...
  \chaptername\ is invalid.}, $p(m,b|\ally,I)$ is the \emph{posterior}
...
\chapter{Pruning outliers}\label{sec:outliers}
...
this \chaptername.  The second is to better model your data-point
uncertainties, permitting larger deviations than the Gaussian noise
estimates; this is the subject of
\chaptername~\ref{sec:non-Gaussian}.  Both of these are strongly
...
and not the outcome of justifiable modeling.\note{\emph{Sigma
  ...
  \chaptername~\ref{sec:goodness}, but a better procedure is to
...
(\chaptername~\ref{sec:standard}) to an objective function
(\chaptername~\ref{sec:objective}) to a general expression for a
...
\chapter{Uncertainties in the best-fit parameters}\label{sec:uncertainty}

In the standard linear-algebra method of $\chi^2$ minimization given
in \chaptername~\ref{sec:standard}, the uncertainties in the best-fit
...
relationship has intrinsic scatter and curvature.\note{If you are
  ...
  \chaptername~\ref{sec:objective}), the Bayesian analysis does not
...
\chaptername~\ref{sec:outliers}, the output is a
...
\chapter{Non-Gaussian uncertainties}\label{sec:non-Gaussian}

The standard method of \chaptername~\ref{sec:standard} is only
...
of \chaptername~\ref{sec:outliers}.  We generally recommend
...
justifiable.\note{We don't recommend them, but there are many
  ...
  generative model for the data.  In this \chaptername\ we modeled
  outliers by a method that can almost be interpreted in terms of a
  softer $\chi^2$; in \chaptername~\ref{sec:non-Gaussian} we discuss
...
For example, the generative model in \chaptername~\ref{sec:objective}
...
become
\begin{multline}\label{eq:too_long_1}
...
\chapter{Goodness of fit and unknown uncertainties}\label{sec:goodness}
...
Imagine that the generative model in \chaptername~\ref{sec:objective}
...
\begin{multline}\label{eq:too_long_2}
\chi^2 =
...
\chaptername.  Note that the inference and marginalization would be
...
\chapter{Arbitrary two-dimensional uncertainties}\label{sec:twod}
...
\begin{multline}\label{eq:too_long_3}
...
$(m,b)$ of the line.  As in \chaptername~\ref{sec:objective}, the
...
approximation.  In \chaptername~\ref{sec:scatter}, we consider the
...
\chaptername~\ref{sec:outliers}.  We ask for this in the
\problemnames\ below.
...
fundamental wrong with the generative model of this \chaptername, and
...
systematic uncertainty.  This is unjustified.\note{The difference
  ...
  the procedure given in this \chaptername.  We have already
...
Using the method of this \chaptername, fit the straight line
...
probability $\Pbad$ described in \chaptername~\ref{sec:outliers}.  Use
...
\chaptername~\ref{sec:standard}.  Now transpose the problem and fit
...
\chapter{Intrinsic scatter}\label{sec:scatter}
...
\emph{orthogonal} to the relationship.\note{One confusing issue in the
  ...
  method involving $V$ in this \chaptername).  However it must be made
