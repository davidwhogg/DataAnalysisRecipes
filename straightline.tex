...
addition, to construct an objective function one needs some additional
parameters including $\Pbad$, the \emph{prior} probability that any
individual data point is bad, and $(\Ybad,\Vbad)$, the mean and
variance of the distribution of bad points (in $y$).  All these $N+3$
extra parameters may seem like crazy baggage, but their values can be
\emph{inferred} and \emph{marginalized out} so in the end, this method
introduces \emph{no new parameters}.

This situation is starting to get complex; it sometimes pays in
complex situations to be a Bayesian, so let's go Bayesian.  In this
case, the likelihood is
\begin{eqnarray}\displaystyle
\like &\equiv& \frac{p(\ally|m,b,\allq,\Pbad,\Ybad,\Vbad,I)}{p(\ally|I)}
 \nonumber\\
\like &\propto& \prod_{i=1}^N \left[\frac{1}{\sqrt{2\,\pi\,\sigma_{yi}^2}}
 \,\exp\left(-\frac{[y_i-m\,x_i-b]^2}{2\,\sigma_{yi}^2}\right)\right]^{q_i}
 \nonumber \\ & & \quad\times
 \left[\frac{1}{\sqrt{2\,\pi\,\Vbad}}
 \,\exp\left(-\frac{[y_i-\Ybad]^2}{2\,\Vbad}\right)\right]^{[1-q_i]}
 \nonumber\\
\ln\like &=& K
 - \sum_{i=1}^N \frac{q_i}{2}\,\ln\sigma_{yi}^2
 - \sum_{i=1}^N q_i\,\frac{[y_i-m\,x_i-b]^2}{2\,\sigma_{yi}^2}
 \nonumber \\ & & \quad
 - \sum_{i=1}^N \frac{[1-q_i]}{2}\,\ln\Vbad
 - \sum_{i=1}^N [1-q_i]\,\frac{[y_i-\Ybad]^2}{2\,\Vbad} \quad ,
\end{eqnarray}
...
\like &\propto&
 ...
 \nonumber \\ & & \quad
 \left.+ \frac{\Pbad}{\sqrt{2\,\pi\,\Vbad}}
 \,\exp\left(-\frac{[y_i-\Ybad]^2}{2\,\Vbad}\right)\right]
 \quad ,
\end{eqnarray}
...
parameters $(m,b)$.  Whether to use the binary assignment or mixture
