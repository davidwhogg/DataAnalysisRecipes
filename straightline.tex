...
% - Hogg: should we say something substantial---not superficial---about non-convexity; perhaps initializing trials with RANSAC-like initial conditions; or even put in a problem to optimize the scalar, but considering *only* the lines created by pairs of points.
...
mentors---Gerry Neugebauer---liked to say, ``They are uncertainties,
not errors; if they were \emph{errors}, we would have corrected
them!''  Your data come with \emph{uncertainties}, which are
limitations to your knowledge, not \emph{mistakes}.
...
point gets a parameter in the form of the probability $p_i$ that that
data point is a member of the foreground population.  In the end, the
best-fit parameters of the line (or the posterior distribution for
them) is found by marginalizing over all the background parameters and
all of the individual data-point probabilities
$\setofall{p_i}{i=1}{N}$.  This solution is beautiful, and of wide
application in the experimental sciences, but because it gets into
issues of \emph{density estimation} we consider it beyond the scope of
this \documentname.
\end{comments}
...
the test is more sensitive.  In the case of correctly estimated
...
reason, large $\Delta\chi^2$ values generate considerable confidence.
...
to obtain hints about what might be going wrong.  Part of the reason
...
this problem---what models to consider---lies in the area of
...
uncertainty estimates, any difference between the jackknife,
bootstrap, and traditional uncertainty estimates for the best-fit line
parameters undermines confidence in the validity of the model.  If you
believe the model, then any difference between the jackknife,
bootstrap, and traditional uncertainty estimates undermines confidence
in the data-point uncertainties $\allsigmay$.
...
disbelieve the standard uncertainty estimate.  In these cases you want
either to be measuring the width of your posterior (if you are a
Bayesian) or else using an empirical estimate of the uncertainty such
as jackknife or bootstrap.
...
$\chi^2$ becomes close to its expected value of $[N-2]$ (or should it
be $[N-3]$ since we have added another parameter).  In performing this
