...
% - Hogg: Foreman-Mackey email of 2010-06-25
...
errors and absurdities in the literature.\note{Along the lines of
  ...
  truth is that this decision must be based on the \emph{uncertainty
    properties} of $x$ and $y$. If $x$ has much smaller uncertainties, then
  you must fit $y$ as a function of $x$; if the other way then the
  other way, and if neither has much smaller uncertainties, then that kind of
...
sensitivity to data of different statistical significance.\note{Even
  ...
  uncertainty or noise properties of the data.  In performing this
  standard fit, the investigator is effectively assuming that the $x$
  values have negligible uncertainties; if they do \emph{not}, then
  the investigator is \emph{making a mistake}.}  It can be justified
in one of several ways; the linear algebra justification starts by
noting that you want to solve the equation
\begin{equation}
\mY = \mA\,\mX \quad ,
\end{equation}
...
$\mX$.  The justification of this will have to wait for a discussion
of the objective function, if it comes at all.
