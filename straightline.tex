...
above, find the best-fit straight line $y=m\,x+b$ for the $x$, $y$,
and $\sigma_y$ values in \tablename~??.  Proceed by (dumbly) looping
over every possible (non-trivial) assigment of $q_i$ values.  Make a
...
\problemname~\ref{prob:standard}?  Do you like it better or worse?
For extra credit, mark each data point on your plot with
the fully marginalized probability that the point is good (that is, not
rejected, or has $q=1$).
\end{problem}
