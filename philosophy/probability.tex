...
% - Do we need a very specific example problem to illustrate everything?  It should thread through secs 2, 3.
% - Needs exercises for sections 2 and 3.
...
below, \cite{sivia} is a great, practical introduction and the first
few chapters of \cite{jaynes} make a great and exceedingly
idiosyncratic read.
...
$a_n$, indexed by $n$, each of which is independent in this sense.  If
you have a set of $N$ independently drawn data $a_n$, each drawn from
a probability distribution $p(a_n \given c)$, then the probability of
the full data set is simply the product of the individual data-point
probabilities:
\begin{eqnarray}\displaystyle
...
\equationnames~(\ref{eq:example0}) through (\ref{eq:example1}) from
the previous \sectionname, if we want to learn the posterior pdf over
the parameters $\theta \equiv [a, b]$, we need to choose a prior pdf
$p(\theta \given I)$.  In principle this should represent our actual
prior or exterior knowledge.  In practice, investigators often want to
``assume nothing'' and put a very or infinitely broad prior on the
parameters; of course putting a broad prior is not equivalent to
assuming nothing, it is just as severe an assumption as any other
prior.  For example, even if you go with a very broad prior on the
parameter $a$, that is a \emph{different} assumption than the same
form of very broad prior on $a^2$ or on $\arctan(a)$.  The prior
doesn't just set the ranges of parameters, it places a \emph{measure
  on parameter space.}  That's why it is so dangerous (and unloved by
frequntists).\note{The insertion of the prior pdf absolutely and
  necessarily increases the number of assumptions; there is no
  avoiding that.  Bayesians sometimes like to say that frequentists
  make just as many assumptions as Bayesians do.  It isn't true: A
  principled frequentist---an investigator who only uses the
  likelihood function and nothing else---genuinely makes fewer
  assumptions than any Bayesian.  The Bayesian gets new capabilities
  in exchange for these new assumptions, but they \emph{are} new.}
