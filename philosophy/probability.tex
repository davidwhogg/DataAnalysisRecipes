...
constructing likelihoods, marginalized likelihoods, and posterior
probabilities are all discussed, briefly.
\end{abstract}
...
\\
p(D \given \theta, I) &=& \prod_{n=1}^N p(D_n \given \theta, I)
\\\displaystyle\label{eq:examplelike}
\theta &\equiv& [a, b]
\\
I &\equiv& [\setofall{x_n, \sigma_n^2}{n=1}{N}, \mbox{and so on\ldots} ]
\quad,\label{eq:example1}
\end{eqnarray}
...
contributions, which are drawn independently from Gaussians $N()$ with
zero means and variances $\sigma_n$, the likelihood is just a Gaussian
...
$\sigma_n^2$ values are considered prior information.\note{In general,
  ...
  $\sigma^2_n$, make them model parameters!  Same with the horizontal
  positions $x_n$.}
...
properties of Gaussians and properties of Gaussians.
\end{problem}
...
Continuing our example from the previous \sectionname \ldots if we put
down a prior PDF of the form \ldots the posterior PDF is \ldots
...
out the $\theta$ using the prior $p(\theta \given \beta, I)$ and be
left with a likelihood for the parameters $\beta$ of the prior, like so:
\begin{eqnarray}\displaystyle
