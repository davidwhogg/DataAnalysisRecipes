...
\section*{DRAFT 2012-03-17:\ Data analysis recipes:\ \\
...
independently drawn data $a_n$, each drawn from a probability
distribution $p(a_n \given c)$, then the probability of the full data
set is simply the product of the individual data-point probabilities:
\begin{eqnarray}\displaystyle
...
\quad .
\end{eqnarray}
This conditional PDF for the data is called the ``likelihood'' for
historical reasons I don't care about.  Worse, it is called the
likelihood ``for the parameters'' even though it is a PDF ``for the
data''.\note{I hate the terminology ``likelihood'' and ``likelihood
  for parameters'' but again, it is so entrenched, it would be a
  disservice to the reader not to use it.}

Now imagine there are important parameters and nuisance parameters \ldots priors on nuisance parameters

Now imagine that you want the probability for the important parameters \ldots more priors.

Now imagine that you want to take expectation values of functions of parameters \ldots
