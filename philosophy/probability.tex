...
Because the data are independent (the full-data likelihood is a
product of individual-datum likelihoods), you might be tempted to do
things like
...
to put it, even if the data are independent, they are not independent
\emph{conditional on $\alpha$}.  Or another way to put it, although
you don't want to infer the $\alpha$ parameters, you want the support
in the marginalization integral to be set by \emph{all} the data taken
together, not each individual datum separately.
...
precisely.  So no single number will adequately represent the result
of a data analysis.  Results are always pdfs (or likelihood
functions); we should embrace that.
